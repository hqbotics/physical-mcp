...
- `/health` and MCP `get_camera_health()` intentionally share normalized `CameraHealth` key-set parity (including nullable fields like `backoff_until`, `last_success_at`, `last_frame_at`).
...
- **Monitoring appears stale**
  ...
  - Delivery timing: startup-warning replay/event-bus entries are emitted during server startup (before any MCP tool call); PMCP session log delivery may replay later after first MCP session attach, with the same `event_id`.
  ...
  - Treat both examples as the canonical subscriber-validation pair for startup/runtime startup_warning behavior.
